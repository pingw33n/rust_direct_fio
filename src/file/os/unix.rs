...
pub fn read_at(file: &File, pos: u64, buf: &mut [u8]) -> Result<usize> {
    check_err_size(unsafe {
         libc::pread(file.as_raw_fd(), buf.as_mut_ptr() as *mut _, buf.len() as _, pos as libc::off_t)
    })
}

pub fn write_at(file: &File, pos: u64, buf: &[u8]) -> Result<usize> {
    check_err_size(unsafe {
        libc::pwrite(file.as_raw_fd(), buf.as_ptr() as *const _, buf.len() as libc::size_t, pos as libc::off_t)
    })
}

pub(in super) fn check_err(r: libc::c_int) -> Result<libc::c_int> {
    if r == -1 {
