...
use crate::cache::{self, PageId, Scope};
...
impl File {
    ...
    pub async fn page(&self, id: PageId) -> Result<PageRef> {
        self.scope.page(id).await.map(PageRef)
    }
    ...
    pub async fn sync_all(&self, sync: FileSync) -> Result<()> {
        scope::sync_all(&self.scope, sync).await
    }

    pub async fn sync_data(&self, sync: FileSync) -> Result<()> {
        scope::sync_data(&self.scope, sync).await
    }
    ...
    pub async fn write_at(&self, mut pos: u64, mut buf: &[u8]) -> Result<()> {
        while buf.len() > 0 {
            let (id, offset) = self.page_id_at(pos);
            let page = self.page(id).await?;
...
mod test {
    ...
    fn new(max_resident: usize, max_non_resident: usize, page_len_scale: usize) -> FileSystem {
        FileSystem::new(Options::default()
    ...
    async fn read_vec_at(file: &File, pos: u64, len: usize) -> Vec<u8> {
        let mut r = Vec::new();
        r.resize(len, 0);
    ...
    fn read_write_on_non_boundary() {
        let mut tmpf = test_block_on(async {
            let fs = new(1, 0, 1);
            ...
            {
                let f = fs.open(tmpf2.path()).await.unwrap();
